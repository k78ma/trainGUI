...
class BertForMultiLable(BertPreTrainedModel):
    def __init__(self, config):
        ...
        self.dimension_reduction = nn.Linear(config.hidden_size, 16)
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None,head_mask=None):
        ...
        pooled_output = self.dimension_reduction(pooled_output) # need to output


        pooled_output_drop = self.dropout(pooled_output)
        logits = self.classifier(pooled_output_drop)
