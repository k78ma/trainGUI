...
def run_test(args):
    ...
    predictor = Predictor(model=model,
                          logger=logger,
                          n_gpu=args.n_gpu)
    result, last_layer_feature = predictor.predict(data=test_dataloader)
    print(result.shape)
    print(last_layer_feature.shape)
    # np.savetxt('pybert/dataset/test_result_output.txt', result)
    # np.savetxt('pybert/dataset/test_last_layer_feature_output.txt', last_layer_feature)
    # result.to_txt('pybert/dataset/test_output.csv', index=False)
    #######################################################postprocessing##############################################
    print('data postprocessing')
    test_input_original = pd.read_csv('pybert/dataset/test.csv', encoding='ISO-8859-1')
    test_feature_output = last_layer_feature

    all_data = pd.read_csv('pybert/dataset/raw/raw_all.csv', encoding='ISO-8859-1')
    # for d in range(test_feature_output.shape[1]):
    #     print(d)

    d = test_feature_output.shape[1]

    test_input = test_input_original
    if test_input.shape[0] == test_feature_output.shape[0]:
        print('Starting output')
        print(test_feature_output.shape)

        for k in range(d):
            # for k in range(2):
            testList = []
            channel = 'Text_Feature' + str(k)
            for i in range(test_feature_output.shape[0]):
                testList.append(test_feature_output[i][k])

            test_input[channel] = testList

        test_input_name = 'pybert/dataset/test_feature_output_' + str(d) + '.csv'
        test_input.to_csv(test_input_name, index=False)

    else:
        print('Wrong: Different size')


    test_feature = test_input
    j = 0
    for i in range(all_data.shape[0]):
        # print(i)
        if all_data['id'][i] != test_feature['id'][j]:
            for k in range(test_feature.shape[1] - 2):
                channel = 'Text_Feature' + str(k)
                all_data.at[i, channel] = ''
        else:
            for k in range(test_feature.shape[1] - 2):
                channel = 'Text_Feature' + str(k)
                all_data.at[i, channel] = test_feature[channel][j]
            j += 1

    all_data_name = 'pybert/dataset/all_output' + str(d) + '.csv'
    all_data.to_csv(all_data_name, index=False)
