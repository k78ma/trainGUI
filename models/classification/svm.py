...
# SVM classification for categorical features
import pdb
...
# TODO: Error importing AbstractModel when run 'python svm.py' for unit test
...
class svm(AbstractModel):
    ...
    def __init__(self, Cs, gammas, kernels, dfs, seed, val_ratio=0.2):

        super().__init__()

        self.classifier = SVC(class_weight="balanced", probability=True, random_state = 45)
        ...
        self.param_grid = {'kernel':kernels,'C':Cs, 'gamma':gammas, 'decision_function_shape':dfs}
        
        self.val_prop = val_ratio
       
    def train(self, x_train, y_train, x_val, y_val):

        inner_cv = KFold(n_splits=4, shuffle=True, random_state=self.seed)
        score1 = 'f1_micro'
        grid_svm = GridSearchCV(self.classifier, self.param_grid, cv = inner_cv, scoring=score1, n_jobs = -1, verbose=1)
        grid_svm.fit(x_train,y_train)

        self.best_estimator = grid_svm.best_estimator_
        self.best_params = grid_svm.best_params_
...
if __name__ == '__main__':
    ...
    my_svm = svm(Cs, gammas, kernels, dfs, seed, val_prop=val_prop)

    #dataset_train = (x_train, y_train)

    my_svm.train(x_train_t, y_train_t, x_train_v, y_train_v)

    ######## 5. test model
    #dataset_test = (x_test, y_test)
    
    my_svm.test(x_test, y_test)
